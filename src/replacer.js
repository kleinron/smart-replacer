...
class MapReplacer {
    ...
    replace = (key, value) => {
        return Array.from(value.entries())
            .reduce((agg, [k, v]) => {agg[k] = v; return agg;}, {});
    }
}
...
class Replacer {
    ...
    static createReplacerFunction(options) {
        ...
        const opts = {...{
            replacers: [],
            withBuiltInReplacers: true,
            monkeyPatchJSON: false
        }, ...options};
        const customReplacers = opts.replacers;
        const withBuiltInReplacers = opts.withBuiltInReplacers;
        const builtInReplacers = withBuiltInReplacers ? createCoreReplacers() : [];
